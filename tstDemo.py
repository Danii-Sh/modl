...
subDirectory='21Jul_1035PM_5L_1K_50E_AG'
...
tstOrg,tstAtb,tstA = sf.getTestingData()
...
with tf.compat.v1.Session(config=config) as sess:
    ...
    AT =graph.get_tensor_by_name('A:0')
    atbT=graph.get_tensor_by_name('atb:0')
    print ('in test', atbT.shape,tstAtb.shape, AT.shape, tstA.shape)
    wts=sess.run(tf.compat.v1.global_variables())
    dataDict={atbT:tstAtb,AT:tstA }
...
print ('PSNR', tstOrg.shape, tstAtb.shape,rec.shape, tstOrg.dtype)


normOrg=sf.normalize01( np.abs((tstOrg.astype('float32'))))
normAtb=sf.normalize01( np.abs(sf.r2c(tstAtb)))
normRec=sf.normalize01(np.abs(rec))
print ('PSNR', normOrg.shape,normAtb.shape,normRec.shape)
